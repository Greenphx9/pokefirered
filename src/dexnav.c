...
static void DexNavShowFieldMessage(u8 id)
{
	...
	{	
		case FIELD_MSG_NOT_IN_AREA:
			ScriptContext1_SetupScript(SystemScript_DexNav_CannotBeFound);
			break;
		case FIELD_MSG_LOOK_IN_OTHER_SPOT:
			ScriptContext1_SetupScript(SystemScript_DexNav_NotFoundNearby);
			break;
		case FIELD_MSG_TOO_DARK:
			ScriptContext1_SetupScript(SystemScript_DexNav_TooDarkForDexNav);
			break;
		case FIELD_MSG_GOT_AWAY:
			ScriptContext1_SetupScript(SystemScript_DexNav_GotAway);
			break;
		case FIELD_MSG_LOST_SIGNAL:
			ScriptContext1_SetupScript(SystemScript_DexNav_LostSignal);
			break;
		case FIELD_MSG_SNEAK_NEXT_TIME:
			ScriptContext1_SetupScript(SystemScript_DexNav_GotAwayShouldSneak);
			break;
	}
}
...
static u8 DexNavGenerateHiddenAbility(u16 species, u8 searchLevel)
{
    ...
    #ifdef HIDDEN_ABILITY
	if (genAbility && gBaseStats[species].abilities[2] != ABILITY_NONE
	&& GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT)) //Only give hidden ability if Pokemon has been caught before
		return gBaseStats[species].abilities[2];
	else
	{
		//Pick a normal ability of that Pokemon
		u8 ability;
		if (gBaseStats[species].abilities[1] != ABILITY_NONE)
			ability = (randVal & 1) == 0 ? gBaseStats[species].ability1 : gBaseStats[species].abilities[1];
		else
...
static void DexNavDrawBButton(u8* spriteIdAddr)
{
	...
	LoadSpritePalette(&sHeldItemSpritePalette);
	spriteId = CreateSprite(&sBButtonTemplate, 208 + (32 / 2), 148 + (8 / 2), 0);
...
static void PrintDexNavMessage(u8 messageId)
{
	...
	AddTextPrinterParameterized3(WIN_MESSAGE, 1, 2, 8, sDexNav_WhiteText, 0, text);
...
static void PrintGUISpeciesName(u16 species)
{
	...
	AddTextPrinterParameterized3(WIN_SPECIES, 0, 0, 4, sDexNav_BlackText, 0, text);
...
static void PrintGUISearchLevel(u16 species)
{
	...
	AddTextPrinterParameterized3(WIN_SEARCH_LEVEL, 0, 0, 4, sDexNav_BlackText, 0, text);
...
static void PrintGUILevelBonus(u16 species)
{
	...
	AddTextPrinterParameterized3(WIN_LEVEL_BONUS, 0, 0, 4, sDexNav_BlackText, 0, text);
...
static void PrintGUIHiddenAbility(u16 species)
{
	...
	{
		if (species != SPECIES_NONE && gBaseStats[species].abilities[2] != ABILITY_NONE)
			text = GetAbilityName(gBaseStats[species].abilities[2]);
		else
			text = gText_DexNav_NoInfo;
	}
	else
...
static void DexNavLoadAreaNamesAndInstructions(void)
{
	...
	AddTextPrinterParameterized3(WIN_LAND, 1, 5, 6, sDexNav_WhiteText, 0, landText);
	AddTextPrinterParameterized3(WIN_WATER, 1, 4, 6, sDexNav_WhiteText, 0, waterText);

	//Print Location Name
	GetMapName(gStringVar4, GetCurrentRegionMapSectionId(), 0);
	AddTextPrinterParameterized3(WIN_MAP_NAME, 1, 2, 0, sDexNav_WhiteText, 0, gStringVar4);

	//Print Instructions
	AddTextPrinterParameterized3(WIN_INSTRUCTIONS, 0, 5, 0, sDexNav_WhiteText, 0, gText_DexNavInstructions);
