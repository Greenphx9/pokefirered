...
void ItemUseCB_Medicine(u8 taskId, TaskFunc func)
{
    ...
    else
    {
        ...
        canHeal = ExecuteTableBasedItemEffect(mon, item, gPartyMenu.slotId, 0);
    }
    PlaySE(SE_SELECT);
    if (canHeal)
    {
...
static void TryUsePPItemOutsideBattle(u8 taskId)
{
    bool8 noEffect = ExecuteTableBasedItemEffect(&gPlayerParty[gPartyMenu.slotId],
                                            gSpecialVar_ItemId,
                                            gPartyMenu.slotId,
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc func)
{
    ...
    if (GetMonData(mon, MON_DATA_LEVEL) != MAX_LEVEL)
        noEffect = ExecuteTableBasedItemEffect(mon, item, gPartyMenu.slotId, 0);
    else
        noEffect = TRUE;
    PlaySE(SE_SELECT);
    if (noEffect)
...
static bool8 MonCanEvolve(void)
{
    // TODO: Fully implement P_NATIONAL_DEX_EVOLUTIONS
...
u8 GetItemEffectType(u16 item)
{
    ...
    if ((itemEffect[0] & (ITEM0_DIRE_HIT | ITEM1_X_ATTACK)) || itemEffect[1] || itemEffect[2] || (itemEffect[3] & ITEM3_GUARD_SPEC))
...
static void TryTutorSelectedMon(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (CanTeachMove(mon, gSpecialVar_0x8005))
        {
        case CANNOT_LEARN_MOVE:
            DisplayLearnMoveMessageAndClose(taskId, gText_PkmnCantLearnMove);
            return;
...
extern void ShiftMoveSlot(struct Pokemon *mon, u8 slotTo, u8 slotFrom);
...
static void Task_TryItemUseFormChange(u8 taskId)
{
    ...
    case 3:
        ...
        {
            DestroyMonIcon(icon);
            CreatePartyMonIconSpriteParameterized(targetSpecies, GetMonData(mon, MON_DATA_PERSONALITY, NULL), &sPartyMenuBoxes[gPartyMenu.slotId], 1, FALSE);
...
static void ItemEffectToStatString(u8 effectType, u8 *dest)
{
    ...
    case ITEM_EFFECT_HP_EV:
        ...
        break;
    case ITEM_EFFECT_ATK_EV:
        StringCopy(dest, gText_LevelUp_Attack);
        break;
    case ITEM_EFFECT_DEF_EV:
        StringCopy(dest, gText_LevelUp_Defense);
        break;
    case ITEM_EFFECT_SPEED_EV:
        StringCopy(dest, gText_LevelUp_Speed);
        break;
    case ITEM_EFFECT_SPATK_EV:
        StringCopy(dest, gText_LevelUp_SpAtk);
        break;
    case ITEM_EFFECT_SPDEF_EV:
        StringCopy(dest, gText_LevelUp_SpDef);
        break;
    }
}